...
namespace Consoler
{
    class Game
    {
        ...
        public static Dictionary<string, ConsoleColor> skinDict = new()
        {
        ...
        public static async Task CommandInput()
        {
            ...
            switch (command)
            {
                case "work":
                    ...
                    {
                        int workTime = Game.r.Next(100, 150);
                        Console.CursorVisible = false;
                        ...
                        Console.CursorVisible = true;
                        float moneyGained = (float)Game.r.NextDouble() * Game.r.Next(5 * Player.level, 100 * (Player.level) / 2);
                ...
                case "study" or "learn":
                    ...
                    {
                        Console.CursorVisible = false;
                        int studyTime = Game.r.Next(150, 200);
                        int experienceGained = Game.r.Next(10, 50 * Player.level / 4);
                ...
                case string s when s.StartsWith("skin"):
                    List<string> z = s.Split(' ').ToList();
                    z.RemoveAt(0);

                    string r = string.Join(" ", z).ToLower();
                    ...
                    if (skinDict.ContainsKey(r))
                    {
                        if(Player.playerInventory.ContainsKey(r))
                        {
                            Console.ForegroundColor = skinDict[r];
                        }
                        else
                        {
                            Console.WriteLine("You dont own that skin!");
                        }
                    }
                    else
                    {
                ...
                case "crime":
                    ...
                    int rolledNumber = Game.r.Next(0, 100);
                    if (rolledNumber > chanceToGetCaught)
                    {
                        new Thread(Player.CrimeDelay).Start();
                        float moneyGained = (float)Game.r.NextDouble() * Game.r.Next(50, 100 * Player.level / 2);
                        int experienceGained = Game.r.Next(25, 100 * Player.level / 4);
                    ...
                    else
                    {
                        ...
                        int moneyLost = Game.r.Next(50, 50 * Player.level);
                ...
                case "stat":
                    ...
                    if (Player.playerInventory.Count > 0)
                    {
                        ...
                        foreach (var item in Player.playerInventory)
                        {
                            ...
                            Console.ForegroundColor = ConsoleColor.Green;
                            Console.Write($"\t\t{UNDERLINE}{item.Value:0.00}${RESET}");
                        }
                        Console.ForegroundColor = ConsoleColor.Magenta;
                        Console.WriteLine("\n---------------------------------------------------------");
                        Console.ForegroundColor = ConsoleColor.White;
                ...
                case string s when s.StartsWith("game") && s.Split(' ', StringSplitOptions.RemoveEmptyEntries)[1].Trim() is "num":
                    int correctNumber = Game.r.Next(0, 10);
                    Console.WriteLine("Guess a number between 0 - 10");
                    int puzzleInput = int.Parse(Console.ReadLine());
                    if (puzzleInput == correctNumber)
                    {
                        int expGranted = Game.r.Next(0, 10 * Player.level / 4);
                        int moneyGranted = Game.r.Next(25, 100 * Player.level);
    ...
    class Player
    {
        ...
        public static void PlaytimeExperience()
        {
            ...
            {
                Thread.Sleep(15000);
                GrantExperience(Game.r.Next(10, 25 * level / 2));
            }
        }
